...
process.on('SIGINT', async () => {
    // Do NOT call process.exit() here — index.js decides whether to exit.
    // On Wispbyte (non-TTY), SIGINT is a platform management signal and
    // the process must stay alive. Just flush data to disk.
    await gracefulShutdown('SIGINT');
});
process.on('SIGTERM', async () => {
    // Do NOT call process.exit() here — index.js ignores SIGTERM intentionally
    // so Wispbyte can send SIGTERM for container management without killing the bot.
    // Just flush data to disk; the process stays alive.
