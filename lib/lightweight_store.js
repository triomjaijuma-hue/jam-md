            ...
            try {
                if (fs.existsSync(filePath)) {
                    ...
                    if (data[chatId] !== undefined)
                        return data[chatId];
                    // If no chatId key found and no JID-keyed entries, the whole object IS the value
                    const hasJidKeys = Object.keys(data).some(k => k.includes('@'));
                    if (!hasJidKeys) return data;
                    return null;
                }
                return null;
            }
