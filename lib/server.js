...
export function startKeepAlive() {
      const INTERVAL_MS = 45 * 1000; // 45s — must be shorter than Wispbyte's ~1-2 min idle timeout
    ...
    // First ping at 10s (let server fully start), then every 45s
    setTimeout(doPing, 10 * 1000);
    setInterval(doPing, INTERVAL_MS);
    console.log('[keep-alive] Started — pinging every 45s to', _detectedPublicUrl || 'localhost fallback');
}
