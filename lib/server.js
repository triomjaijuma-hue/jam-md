...
// Pending pairing code — set by index.js, shown on /pair page
...
<title>TRAILER-UPS Status</title><style>${SHARED_CSS}
  ...
  <div class="logo">TRAILER-UPS</div>
  ...
  <div class="grid">
    ...
    <div class="row"><span class="lbl">Owner</span><span class="val">${config.botOwner||'TrailerUps Dev'}</span></div>
  </div>
  <div class="nav">
...
app.get('/pair', (req, res) => {
    ...
    res.send(`<!DOCTYPE html><html lang="en"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>TRAILER-UPS — Pair Device</title><style>${SHARED_CSS}</style></head><body>
<div class="card">
  <div class="logo">TRAILER-UPS</div>
  <p class="sub">WhatsApp Pairing — by TrailerUps Dev</p>
  ...
  <input type="tel" id="phone" placeholder="256706106326" autocomplete="off"/>
...
async function requestCode(phone,isRetry){
  ...
  try{
    ...
    if(data.code){
      ...
      result.innerHTML=\`<div class="success-box">✅ <b>Bot is already connected!</b><br><br>Your session is saved automatically on the server — no action needed.</div>\`;
    }else{
...
app.post('/pair/request', async (req, res) => {
    try {
        ...
        // Wait up to 12s for the bot socket to be ready (in case of fresh deploy)
        let waited = 0;
        ...
        if (_sock.user) return res.json({ error: 'Bot is already linked and your session is saved locally.' });
        let code = await _sock.requestPairingCode(phone);
        code = code?.match(/.{1,4}/g)?.join('-') || code;
        setPairingCode(code); // also store so page auto-refreshes show it
...
app.get('/session', (req, res) => {
    ...
    res.send(`<!DOCTYPE html><html lang="en"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>TRAILER-UPS — Save Session</title><style>${SHARED_CSS}
...
<div class="card">
  ...
  <div id="content">
    ...
    <div class="info-box mt">
      ...
      <div class="step-row"><span class="step-num">2</span>Your session is <b>already saved</b> on the server in <code>session/creds.json</code></div>
      <div class="step-row"><span class="step-num">3</span>On bot-hosting.net (Pterodactyl), this file <b>persists between restarts</b></div>
      <div class="step-row"><span class="step-num">4</span>No environment variable needed — just restart the bot normally ✅</div>
    </div>
    ` : `
    ...
    <div class="info-box">
      ...
      <div class="step-row"><span class="step-num">4</span>No extra steps needed — session persists automatically on bot-hosting.net ✅</div>
    </div>
    `}
  </div>
...
<script>
${isConnected ? `
// Fetch and display session string
...
loadSession();
` : `
// Auto-refresh every 5s until bot connects
...
app.get('/session/export', (req, res) => {
    try {
        ...
        // Base64 encode of the session credentials
