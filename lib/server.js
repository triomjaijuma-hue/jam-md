...
export function startKeepAlive() {
    ...
    const doPing = async () => {
        // Always prefer the real detected public URL (set by the Host-header middleware above)
        const publicBase = _detectedPublicUrl;
        const localBase  = 'http://localhost:' + PORT;

        // Ping public URL if we have one
        if (publicBase) {
            try {
                ...
                clearTimeout(t);
                if (res.ok) {
                    console.log('[keep-alive] ✅ Public ping OK —', publicBase, '— uptime', Math.floor(process.uptime() / 60) + 'min');
                } else {
                    console.log('[keep-alive] ⚠️ Public ping', res.status, publicBase);
                }
            } catch (e) {
                if (e.name !== 'AbortError') console.log('[keep-alive] ⚠️ Public ping failed:', e.message);
            }
        }

        // Always ping localhost too (keeps process warm even before first external request)
        try {
            await fetch(localBase + '/ping', { signal: AbortSignal.timeout(10000) });
        } catch {}
    };

    // First ping after 20 s (let bot fully start up), then every 3 min
    setTimeout(doPing, 20 * 1000);
    setInterval(doPing, INTERVAL_MS);
    console.log('[keep-alive] Started — pinging every 3 min. Public URL auto-detects from first real request.');
}
