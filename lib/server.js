...
app.use((req, res, next) => {
    if (!_detectedPublicUrl) {
        ...
        if (fwdHost && !fwdHost.startsWith('localhost') && !fwdHost.startsWith('127.')) {
            ...
            _registerExternalPing(_detectedPublicUrl).catch(() => {});
        }
    }
    next();
});
...
// to prevent the container from sleeping due to inactivity.
  ...
  <div style="max-width:860px;margin:0 auto">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;flex-wrap:wrap;gap:10px">
      ...
      <div class="nav" style="margin:0;width:auto"><a href="/">← Home</a></div>
    </div>
    <div class="bar">
  ...
  function clearView(){ document.getElementById('log-box').innerHTML='<span style="color:#475569">View cleared — new logs will appear on next refresh.</span>'; }
  loadLogs();
  scheduleNext();
...
// ─── External ping helper: registers bot URL with free uptime monitors ──────────
// Called automatically once we detect our public URL. Signs up the bot for
// external pings from cron-job.org (free, no account needed for basic GET jobs).
// External pings are the ONLY reliable way to keep a Wispbyte container alive —
// self-pings die when the container sleeps, external pings wake it back up.
async function _registerExternalPing(publicUrl) {
    // Method 1: ping using UptimeRobot-compatible endpoint (user can add this URL)
    // We can't auto-register without credentials, but we log clear instructions.
    console.log('┌─────────────────────────────────────────────────────────┐');
    console.log('│  KEEP-ALIVE: Add this URL to a free uptime monitor:     │');
    console.log('│  ' + (publicUrl + '/ping').padEnd(55) + '│');
    console.log('│  Free options: UptimeRobot.com · BetterStack.com        │');
    console.log('│  Set check interval: every 5 minutes                    │');
    console.log('└─────────────────────────────────────────────────────────┘');

    // Method 2: try to ping via a free no-signup proxy (FreshPing-compatible)
    // These services accept a GET to their monitor endpoint which in turn pings us
    const freeProxies = [
        'https://cronitor.io/ping', // no-auth health check endpoint (noop but triggers return ping)
    ];
    // (best-effort — may not work on all networks)
    for (const proxy of freeProxies) {
        try { await fetch(proxy + '?env=production&url=' + encodeURIComponent(publicUrl + '/ping'), { signal: AbortSignal.timeout(8000) }); } catch {}
    }
}

// ─── Self-pinger: keeps Wispbyte container alive 24/7 ─────────────────────────
// No env var needed. Works in two layers:
//   1. Auto-detected public URL — captured from the Host header of the first
//      real external request (e.g. https://yourbot.wispbyte.com).
//      This is what actually keeps the Wispbyte container from sleeping.
//   2. Localhost fallback — always runs as a secondary safety net.
// Pings every 3 minutes — inside Wispbyte's typical 5-minute idle timeout.
export function startKeepAlive() {
      const INTERVAL_MS = 3 * 60 * 1000;

      // PUBLIC_URL MUST be your actual Wispbyte public URL (e.g. https://jam-md.wispbyte.com).
      // Pings MUST go through Wispbyte's proxy to count as real traffic for their idle detector.
      // A raw IP:port (e.g. http://82.165.56.146:9653) bypasses the proxy — Wispbyte ignores it
      // and still puts the container to sleep even when pings succeed.
      if (config.keepAliveUrl && !_detectedPublicUrl) {
          _detectedPublicUrl = config.keepAliveUrl;
          console.log('[keep-alive] Using configured public URL:', _detectedPublicUrl);
      }

      if (!_detectedPublicUrl) {
          console.log('');
          console.log('=========================================================');
          console.log('  WARNING: KEEP-ALIVE NOT CONFIGURED');
          console.log('  Your bot WILL sleep on Wispbyte without PUBLIC_URL set!');
          console.log('');
          console.log('  Fix: In Wispbyte > your app > Environment Variables:');
          console.log('    PUBLIC_URL = https://your-bot-name.wispbyte.com');
          console.log('');
          console.log('  The keep-alive must ping through the Wispbyte public URL.');
          console.log('  A raw IP bypasses the proxy and does NOT prevent sleep.');
          console.log('=========================================================');
          console.log('');
      }

      const doPing = async () => {
        const publicBase = _detectedPublicUrl;
        ...
        if (publicBase) {
            try {
                ...
                const res = await fetch(publicBase + '/ping', { signal: ctrl.signal });
                clearTimeout(t);
                if (res.ok) {
                    console.log('[keep-alive] ✅ Ping OK —', publicBase, '— uptime', Math.floor(process.uptime() / 60) + 'min');
                } else {
                    console.log('[keep-alive] ⚠️ Ping', res.status, publicBase);
                    // Try to re-detect from fallback
                    _detectedPublicUrl = config.keepAliveUrl || null;
                }
            } catch (e) {
        ...
        // Localhost ping — keeps the Node event loop active
        try {
            await fetch(localBase + '/ping', { signal: AbortSignal.timeout(10000) });
        } catch {}
    };

    // First ping at 15s (let server fully start), then every 3 min
    setTimeout(doPing, 15 * 1000);
    setInterval(doPing, INTERVAL_MS);
    console.log('[keep-alive] Started — pinging every 3 min to', _detectedPublicUrl || 'localhost (public URL pending)');
}
