...
app.get('/', (req, res) => {
    ...
    const _pc = (_pendingCode && Date.now() < _pendingCodeExpiry) ? _pendingCode : null;
...
async function requestCode(){
  ...
  if(!phone||phone.length<7){result.style.display='block';result.innerHTML='<div class="error-box">⚠️ Enter a valid phone number.</div>';return;}
  ...
  try{
    ...
    if(data.code){
      result.innerHTML=\`<div class="code-box">
        ...
        <div class="steps">
          <b>How to link:</b><br>
          1. Open WhatsApp → tap ⋮ → Linked Devices<br>
          2. Tap "Link a Device"<br>
          3. Tap "Link with phone number instead"<br>
          4. Enter this code within 60 seconds<br><br>
          After linking, visit <a href="/session" style="color:var(--g)">Save Session</a> to get your SESSION_ID.
        </div>
      </div>\`;
    }else{result.innerHTML=\`<div class="error-box">❌ \${data.error||'Failed. Try again.'}</div>\`;}
  }catch(e){
    spin.style.display='none';btn.disabled=false;btn.textContent='Get Pairing Code';result.style.display='block';
    result.innerHTML='<div class="error-box">❌ Network error.</div>';
  }
}
...
<div class="card">
  ...
  <div id="content">
    ...
    <div class="info-box mt">
      ...
      <div class="step-row"><span class="step-num">2</span>Open Railway → your service → <b>Variables</b></div>
      <div class="step-row"><span class="step-num">3</span>Add or update: <b>SESSION_ID</b> = paste the value</div>
      <div class="step-row"><span class="step-num">4</span>Railway will restart the bot with this saved session</div>
      <div class="step-row"><span class="step-num">5</span>Next time the container restarts, no re-pairing needed ✅</div>
    </div>
    ` : `
    ...
    <div class="info-box">
      ...
      <div class="step-row"><span class="step-num">4</span>Copy it and set it in Railway Variables</div>
    </div>
    `}
  </div>
...
app.get('/session/export', (req, res) => {
    try {
        ...
        // Base64 encode — this is the SESSION_ID value the user sets in Railway
