#!/bin/bash
# JAM-MD Heroku One-Click Deployer
# Usage: bash <(curl -s https://raw.githubusercontent.com/JAM-MD/JAM-MD/main/lib/heroku.sh)
...
# Session ID
read -p "$(echo -e ${CYAN}Session ID (JAM-MD/JAM-MD_xxxxx): ${NC})" SESSION_ID
...
if [ ! -f "heroku.yml" ]; then
    ...
    git clone https://github.com/JAM-MD/JAM-MD jam-md-deploy
    cd jam-md-deploy
fi
