...
import { handleAiOnResponse } from '../plugins/aion.js';
...
async function handleMessages(sock, messageUpdate) {
    try {
        ...
        try {
            ...
            // Stealth mode check failed — skip autoread to avoid accidentally
            // marking messages as read when we cannot confirm stealth is off.
            printLog('warning', 'Could not verify stealth mode — skipping autoread for safety');
        }
        ...
        // ── Game reply routing (runs before command processing) ────────────────
        // Trivia: plain A / B / C / D
        if (/^[abcd]$/i.test(userMessage)) {
            const { handleTriviaReply } = await import('../plugins/trivia.js');
            const handled = await handleTriviaReply(sock, chatId, senderId, userMessage.toUpperCase(), message, channelInfo);
            if (handled) return;
        }
        // Video download quality: plain 1 – 5
        if (/^[1-5]$/.test(userMessage)) {
            const { handleVdownReply } = await import('../plugins/vdown.js');
            const handled = await handleVdownReply(sock, chatId, parseInt(userMessage, 10), message, channelInfo);
            if (handled) return;
        }
        // Math difficulty: plain word (easy / hard / …)
        {
            const { handleMathModeReply } = await import('../plugins/math.js');
            const handled = await handleMathModeReply(sock, chatId, userMessage, message);
            if (handled) return;
        }
        // TicTacToe moves: 1–9 and surrender
        ...
        // ── Forward tracker: silently record every media message ──────
        try {
            const { trackForwardedMessage } = await import('./forwardTracker.js');
            trackForwardedMessage(message, senderId, message.pushName || '', chatId, isGroup);
        } catch (_ftErr) {}
        ...
        if (!usedPrefix && !command) {
            ...
            // AI-on mode: reply to every message in enabled chats (groups + DMs)
            const aiOnHandled = await handleAiOnResponse(sock, chatId, message, userMessage, senderId);
            if (aiOnHandled) return;
            if (isGroup) {
        ...
        if (!command) {
            // AI-on mode fallback path
            const aiOnHandled2 = await handleAiOnResponse(sock, chatId, message, userMessage, senderId);
            if (aiOnHandled2) return;
            if (isGroup) {
                await handleTagDetection(sock, chatId, message, senderId);
                await handleMentionDetection(sock, chatId, message);
                const botMode = await store.getBotMode();
    ...
    try {
        ...
        switch (action) {
            ...
            case 'remove':
                ...
                // Track if the owner left so .rejoin can work
                try {
                    const ownerNum = (config.ownerNumber || '').replace(/[^0-9]/g, '');
                    const leftOwner = participants.some(p => {
                        // Strip @domain AND :device suffix → bare phone number
                        const pNum = p.split('@')[0].split(':')[0];
                        return pNum === ownerNum;
                    });
                    if (leftOwner) {
                        let groupName = id;
                        try { const meta = await sock.groupMetadata(id); groupName = meta.subject || id; } catch (_e) {}
                        const { dataFile: _df, DATA_DIR: _dd } = await import('./paths.js');
                        if (!fs.existsSync(_dd)) fs.mkdirSync(_dd, { recursive: true });
                        fs.writeFileSync(_df('last_left_group.json'), JSON.stringify({
                            groupId: id, groupName, leftAt: Date.now()
                        }));
                        printLog('info', 'Owner left group saved for .rejoin: ' + groupName);
                    }
                } catch (_rjErr) { printLog('warning', 'rejoin tracker: ' + _rjErr.message); }
                break;
            default:
