...
import { handlePerChatAi } from '../plugins/aion.js';
import { handleIncomingImageAI } from './visionAI.js';
...
async function handleMessages(sock, messageUpdate) {
    try {
        ...
        // Fire-and-forget: archiving must never block command processing
        ...
        const isImageMsg = !!(message.message?.imageMessage);
        ...
        if (!usedPrefix && !command) {
            ...
            // Image message: try Gemini Vision first (works with or without caption)
            if (isImageMsg && !message.key.fromMe) {
                const imageHandled = await handleIncomingImageAI(sock, chatId, message, senderId, isGroup);
                if (imageHandled) return;
                // If vision didn't handle it (no AI active / no Gemini key):
                // fall through — caption (if any) will be caught by text routing below
            }
            if (!message.key.fromMe && userMessage) {
                const aiHandled = await handlePerChatAi(sock, chatId, message, userMessage, senderId);
                if (aiHandled) return;
            }
            if (isGroup) {
        ...
        if (!command) {
            // Image message: try Gemini Vision first
            if (isImageMsg && !message.key.fromMe) {
                const imageHandled = await handleIncomingImageAI(sock, chatId, message, senderId, isGroup);
                if (imageHandled) return;
            }
            ...
            try {
                ...
                if (!antiCallNotified.has(callerJid)) {
                    ...
                    await sock.sendMessage(callerJid, {
                        text: `Please am busy, I'll call you when am free 🙏`
                    });
