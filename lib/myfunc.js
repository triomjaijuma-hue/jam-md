...
// sharp is optional — lazily loaded so bot starts even if sharp isn't installed
let _sharp = null;
async function getSharp() {
    if (!_sharp) {
        try { _sharp = (await import('sharp')).default; }
        catch { throw new Error('sharp not installed — image commands unavailable on this platform'); }
    }
    return _sharp;
}
// ─── Utilities ───────────────────────────────────────────────────────────────
...
export const reSize = async (buffer, ukur1, ukur2) => (await getSharp())(buffer).resize(ukur1, ukur2).jpeg().toBuffer();
// ─── Size ─────────────────────────────────────────────────────────────────────
...
export const smsg = (JamBot, m, store) => {
    if (!m)
        return m;
    ...
    if (m.key) {
        ...
        m.sender = JamBot.decodeJid(m.fromMe && JamBot.user.id || m.participant || m.key.participant || m.chat || '');
        if (m.isGroup)
            m.participant = JamBot.decodeJid(m.key.participant) || '';
    }
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            m.quoted.sender = JamBot.decodeJid(m.msg.contextInfo.participant);
            m.quoted.fromMe = m.quoted.sender === (JamBot.user && JamBot.user.id);
            m.quoted.text = m.quoted.text || m.quoted.caption || m.quoted.conversation || m.quoted.contentText || m.quoted.selectedDisplayText || m.quoted.title || '';
            m.quoted.mentionedJid = m.msg.contextInfo ? m.msg.contextInfo.mentionedJid : [];
            m.getQuotedObj = m.getQuotedMessage = async () => {
                if (!m.quoted.id)
                    return false;
                const q = await store.loadMessage(m.chat, m.quoted.id, JamBot);
                return smsg(JamBot, q, store);
            };
            ...
            m.quoted.delete = () => JamBot.sendMessage(m.quoted.chat, { delete: vM.key });
            m.quoted.copyNForward = (jid, forceForward = false, options = {}) => JamBot.copyNForward(jid, vM, forceForward, options);
            m.quoted.download = () => JamBot.downloadMediaMessage(m.quoted);
        }
    }
    if (m.msg?.url)
        m.download = () => JamBot.downloadMediaMessage(m.msg);
    m.text = m.msg?.text || m.msg?.caption || m.message?.conversation || m.msg?.contentText || m.msg?.selectedDisplayText || m.msg?.title || '';
    m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text)
        ? JamBot.sendMedia(chatId, text, 'file', '', m, { ...options })
        : JamBot.sendText(chatId, text, m, { ...options });
    m.copy = () => smsg(JamBot, M.fromObject(M.toObject(m)), store);
    m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => JamBot.copyNForward(jid, m, forceForward, options);
    return m;
};
