...
export async function askAI(prompt) {
    ...
    try {
        ...
        // Rate limited — try another configured premium provider automatically
        const fallbacks = Object.keys(PROVIDERS).filter(name =>
            name !== providerName &&
        ...
        for (const fallbackName of fallbacks) {
            try {
                const fallbackKey = state.keys?.[fallbackName] || process.env[`${fallbackName.toUpperCase()}_API_KEY`];
                return await PROVIDERS[fallbackName].ask(prompt, fallbackKey);
            } catch { continue; }
        }
        // No working fallback found — rethrow the rate limit error
        throw err;
    }
}
