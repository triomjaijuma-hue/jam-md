...
// ─── Persona ───────────────────────────────────────────────────────────────
const SYSTEM_PERSONA = `You are a real person chatting with a friend on WhatsApp. You are into movies, TikTok, YouTube, video games, music, pop culture, and internet trends.
Rules:
...
- For questions about movies, series, TikTok trends, YouTube videos, video games, music, celebrities, viral moments — give a real answer like someone who follows these things. Don't pretend you don't know.
- Do NOT bring up football or sports unless the friend specifically asks about it first.
...
// Detect messages that need live/current info before replying
function needsSearch(message) {
    ...
    const triggers = [
        ...
        'twitter', 'tweet', 'tiktok', 'youtube', 'instagram', 'viral video',
        'x.com', 'retweet', 'posted', 'went viral', 'blew up'
    ];
    return triggers.some(t => lower.includes(t));
}

// DuckDuckGo HTML scrape — returns real web snippet results
async function searchDDG(query) {
    try {
        ...
        let m;
        while ((m = rx.exec(data)) !== null && snippets.length < 5) {
...
// YouTube search — finds recent/trending videos on a topic
async function searchYouTube(query) {
    try {
        const yts = (await import('yt-search')).default;
        const { videos } = await yts(query);
        if (!videos?.length) return '';
        const top = videos.slice(0, 3).map(v =>
            `"${v.title}" by ${v.author?.name || 'unknown'} (${v.timestamp || ''})`
        );
        return `YouTube results: ${top.join(' | ')}`;
    } catch { return ''; }
}

// Combine all sources in parallel — each runs independently, failures don't block others
async function searchMultiSource(query) {
    const lower = query.toLowerCase();
    const wantsYT  = lower.includes('youtube') || lower.includes('video') || lower.includes('trend');
    const wantsTW  = lower.includes('twitter') || lower.includes('tweet') || lower.includes('x.com');
    const wantsTT  = lower.includes('tiktok') || lower.includes('tik tok');

    // Build the list of parallel searches
    const jobs = [
        searchDDG(query),                                                           // General web
        wantsYT || wantsTW || wantsTT ? searchYouTube(query) : Promise.resolve(''), // YouTube
        wantsTW ? searchDDG(`site:twitter.com OR site:x.com ${query}`) : Promise.resolve(''), // Twitter/X
        wantsTT ? searchDDG(`site:tiktok.com ${query}`) : Promise.resolve(''),      // TikTok
    ];
...
// Build system prompt — enriched with live multi-source results when needed
async function buildSystemPrompt(userMsg) {
    if (!needsSearch(userMsg)) return SYSTEM_PERSONA;
    const info = await searchMultiSource(userMsg);
    if (!info) return SYSTEM_PERSONA;
    return `${SYSTEM_PERSONA}\n\n[You checked online and found this — use it naturally in your reply as if you just know it, don't say you searched]:\n${info}`;
}
...
const PROVIDERS = {
    ...
    gemini: {
        ...
        async ask(fullPrompt, apiKey) {
            ...
            const userMsg = extractUserMessage(fullPrompt);
            // Only enable Google Search grounding when the message actually needs live data
            // (news, scores, current events etc) — skip for casual chat to avoid delays
            const useSearch = needsSearch(userMsg);
            const payload = {
...
async function getState() {
    ...
    // Fallback: read directly from env vars — populated by dotenv at startup
    // so settings survive crashes and redeploys without re-running .aiswitch/.aikey
    const keys = {};
    ...
    let provider = process.env.AI_PROVIDER || 'mistral';
    if (provider === 'mistral') {
        if (keys.gemini) provider = 'gemini';
