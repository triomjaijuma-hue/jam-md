...
async function getState() {
    ...
    // Fallback: read from env vars / config.js defaults so settings survive
    // every crash and redeploy without needing to run .aiswitch/.aikey again
    const keys = {};
    if (config.groqApiKey) keys.groq = config.groqApiKey;
    if (config.geminiApiKey) keys.gemini = config.geminiApiKey;
    if (config.openaiApiKey) keys.openai = config.openaiApiKey;
    // Also pick up any provider-specific env vars directly
    for (const p of Object.keys(PROVIDERS)) {
        const envKey = process.env[`${p.toUpperCase()}_API_KEY`];
        if (envKey && !keys[p]) keys[p] = envKey;
    }
    // Auto-select the best available provider: prefer premium keys over free
    let provider = config.aiProvider || process.env.AI_PROVIDER || 'mistral';
    if (provider === 'mistral') {
